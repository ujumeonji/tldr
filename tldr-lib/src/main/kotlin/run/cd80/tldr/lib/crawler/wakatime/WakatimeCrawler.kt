package run.cd80.tldr.lib.crawler.wakatime

import com.google.gson.Gson
import run.cd80.tldr.lib.crawler.wakatime.dto.GetTaskRecord
import run.cd80.tldr.lib.crawler.wakatime.response.DurationResponse
...
class WakatimeCrawler constructor(
    ...
    private suspend fun getDurationsFromWakatime(base64ApiKey: String, date: LocalDate): List<GetTaskRecord.Result> =
        try {
            val response = httpClient.get(DURATION_API_ENDPOINT) {
                parameter("date", "$date")
